...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//book:catalog">
    <xsl:element name="html">
      ...
      <xsl:element name="body">
        ...
        <xsl:for-each select="xsl:simpleType[@name='genre']/xsl:restriction/xsl:enumeration">
          <xsl:call-template name="out_genre">
            <xsl:with-param name="genre_name">@value</xsl:with-param>
          </xsl:call-template>
        </xsl:for-each>
  ...
  <xsl:template name="out_genre">
    ...
    <xsl:element name="h1">
      <xsl:value-of select="$genre_name+concat('Books Library Report ', date:today())"/>
    </xsl:element>
